...
struct instruction output[0xFF / 3] = {0};
size_t output_idx = 0;
...
{
    if (output_idx >= sizeof output) {
        fputs("Not enough space left for output!\n", stderr);
        exit(1);
    }
...
void interpret_stk(arg_t arg1, arg_t arg2)
{
    ...
    if (arg1) {
        register_value[arg1] = memory[register_value[sp]];
...
void interpret_jmp(arg_t arg1, arg_t arg2)
{
    ...
    if (arg1 & register_value[flags]) {
        ...
        register_value[ip] = register_value[arg2];
    }
    else {
...
void syscall_open(arg_t arg1, arg_t arg2)
{
    print_std("... filename: %s, flags: %hhb", &memory[register_value[A]],  
                                               register_value[B]);
    register_value[arg2] = open((char*) memory + register_value[A],
                                register_value[B]);
}
