#!/usr/bin/exec-suid -- /usr/bin/python4 -I
...
def is_privileged():
    return os.getuid() == 0 or int(
        open("/run/dojo/sys/workspace/privileged", "r").read()
    )
...
def start():
    bzImage = (
        "/challenge/bzImage"
        if os.path.exists("/challenge/bzImage")
        else "/opt/linux/bzImage"
    )
    ...
    qemu_argv = [
        ...
        f"{cpu},smep,smap",
        "-fsdev",
        "local,id=rootfs,path=/,security_model=passthrough",
        "-device",
        "virtio-9p-pci,fsdev=rootfs,mount_tag=/dev/root",
        "-fsdev",
        "local,id=homefs,path=/home/hacker,security_model=passthrough",
        "-device",
        "virtio-9p-pci,fsdev=homefs,mount_tag=/home/hacker",
        "-device",
        "e1000,netdev=net0",
        "-netdev",
        "user,id=net0,hostfwd=tcp::22-:22",
        "-m",
        "2G",
        "-smp",
        "2" if kvm else "1",
...
def debug():
    ...
    vmlinux = (
        "/challenge/vmlinux"
        if os.path.exists("/challenge/vmlinux")
        else "/opt/linux/vmlinux"
    )
...
def main():
    ...
    build_parser = subparsers.add_parser(
        "build", help="privileged: build vm kernel module"
    )
    build_parser.add_argument("build_path")

    parser.add_argument(
        "--kaslr", dest="nokaslr", action="store_false", help="privileged: enable kaslr"
    )
    parser.add_argument(
        "--no-kaslr",
        dest="nokaslr",
        action="store_true",
        help="privileged: disable kaslr",
    )
    ...
    commands = {
        ...
        "debug": lambda: (
            debug() if not in_vm() else error("Error: cannot debug from within vm")
        ),
